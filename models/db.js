/**
 * Created by newuser on 2017/11/10.
 */
const mongodb = require('mongodb').MongoClient   // 引包
const url=require('./../setting').url   // 数据库地址   setting相当于use命令
...
exports.updateDocument = function(collectionName, filterJson, updateJson, callback) {
    __connectDb((err, db) => {
        ...
        db.collection(collectionNmae).updateMany(filterJson, updateJson, (err, r) => {
            if(err) {
                callback(err, null)
                db.close()
                return
...
exports.find = function(collectionName, json, callback, ...rest) {
    ...
    __connectDb((err, db) => {
        ...
        // 普通查找
        db.collection(collectionName).find(json).skip(skipNumber).sort({time:-1})
